...
- `index.php` — minimal router that includes files from the `scripts/` directory based on the request path.
- `scripts/` — action/view endpoints and per-kid folders (e.g. `scripts/home.php`, `scripts/victor/read.php`).
- `db/piggy.sqlite` — SQLite database file.
...
Navigate to `script` and run the interactive script `create_kid.php` (`php create_kid.php`). You will be prompted for a kid's name, and necessary files will be copied, and index.php and home.php will be ammended accrodingly.
...
- `/{kid}/modify` — form + `POST` handler to insert a new record for that kid (returns JSON on success), e.g. `/victor/modify`.
- `/{kid}/add` — optional helper to add records for a kid (some kids have `add.php` script).
